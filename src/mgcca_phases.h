// mgcca_phases.h — the mgcca pipeline as reusable inline stages, so both the
...
// BigDataStatMeth.hpp and mgcca_io.h.
...
// ---- Presence of individuals in a block ------------------------------------
// getK aligns every table on the union of individual IDs and pads the rows of
// the individuals a table does not hold with zeros, so that K_j X_j = X_j. Those
// padded rows are an algebraic device, not data: any per-block statistic (an SD,
// a degrees-of-freedom count) must be taken over the PRESENT rows only, and the
// per-block scores of the absent individuals are undefined rather than zero.

// Row indices (0-based, into the union) of the individuals present in block ds,
// i.e. the non-zero entries of the diagonal of K_j (MGCCA_TMP/K/<ds>, m x 1).
inline std::vector<int> present_rows(const std::string& filename,
                                     const std::string& tmp_group,
                                     const std::string& ds) {
    Eigen::VectorXd Kd = read_full(filename, tmp_group + "/K", ds).col(0);
    std::vector<int> pres;
    pres.reserve((std::size_t)Kd.size());
    for (int i = 0; i < (int)Kd.size(); ++i) if (Kd(i) != 0.0) pres.push_back(i);
    return pres;
}

// Standardise the block coefficients A_j by the SD of the block scores
// KXA = K_j X_j A_j, computed over the individuals PRESENT in the block. Taking
// it over the whole union would deflate the SD by the zero-padded rows and make
// the returned scores systematically too large.
inline Eigen::MatrixXd scale_by_present_sd(const Eigen::MatrixXd& A,
                                           const Eigen::MatrixXd& KXA,
                                           const std::vector<int>& pres,
                                           int nfac, const std::string& ds) {
    const double npr = (double)pres.size();
    if (npr < 2.0)
        throw std::runtime_error("scores: block '" + ds + "' has fewer than two "
                                 "present individuals; its scores cannot be scaled");
    Eigen::MatrixXd As = A;
    for (int b = 0; b < nfac; ++b) {
        double mean = 0.0;
        for (std::size_t r = 0; r < pres.size(); ++r) mean += KXA(pres[r], b);
        mean /= npr;
        double ss = 0.0;
        for (std::size_t r = 0; r < pres.size(); ++r) {
            const double d = KXA(pres[r], b) - mean;
            ss += d * d;
        }
        As.col(b) /= std::sqrt(ss / (npr - 1.0));
    }
    return As;
}

// Mark the individuals absent from block ds in FINAL_RESULTS/scores/<ds>: their
// rows carry no data, so they are stored as NaN (collected as NA in R) instead of
// the exact 0 that the zero-padded X_j would otherwise produce -- which is
// indistinguishable from a genuinely average measured individual. The matrix is
// m x nfac (nfac tiny), so this stays in the small-object budget of the stage.
inline void mark_absent_scores(const std::string& filename,
                               const std::string& final_group,
                               const std::string& ds,
                               const std::vector<int>& pres,
                               Rcpp::CharacterVector rn, Rcpp::CharacterVector cn) {
    Eigen::MatrixXd S = read_full(filename, final_group + "/scores", ds);
    if ((int)pres.size() < S.rows()) {
        std::vector<char> keep((std::size_t)S.rows(), 0);
        for (std::size_t r = 0; r < pres.size(); ++r) keep[(std::size_t)pres[r]] = 1;
        const double na = std::numeric_limits<double>::quiet_NaN();
        for (int i = 0; i < S.rows(); ++i)
            if (!keep[(std::size_t)i]) S.row(i).setConstant(na);
    }
    write_full(filename, final_group + "/scores", ds, S, 0, rn, cn);
}
...
inline Rcpp::NumericVector run_eigen(const std::string& filename,
    ...
    // The operator is m x m (m = individuals); at most m components exist, and the
    // EXTERNAL gap mu_L - mu_{L+1} needs a (L+1)-th eigenvalue, i.e. nfac < m. Reject
    // nfac > m outright; nfac == m is allowed but the external gap is then undefined.
    if (nfac < 1 || nfac > (int)m)
        throw std::runtime_error("run_eigen: nfac (" + std::to_string(nfac) +
            ") must be in 1.." + std::to_string(m) + " (number of individuals)");
    ...
    // ADAPTIVE symmetric eigensolver on the operator MKsum05 (m x m, m = n = sample
    // size). For the typical mgcca domain (n up to a few thousand) the operator fits
    // comfortably in RAM and a DIRECT LAPACK solver is the correct default: exact, no
    // convergence step, and robust to ill-separated / near-degenerate spectra that
    // defeat the iterative (Spectra) path. The SAME SelfAdjointEigenSolver is used in
    // run_XKX. Only when the operator is too large to materialize do we use the
    // iterative out-of-core path, and even then retry directly on non-convergence.
    // keig = nfac+1 so the EXTERNAL gap mu_L - mu_{L+1} is always defined (when it
    // exists); downstream slices to nfac. N_DIRECT overridable via env for testing.
    int N_DIRECT = 8192;                                  // hard dimension cap
    { const char* ev = std::getenv("MGCCA_EIGEN_DIRECT_MAX");
      if (ev && *ev) { int v = std::atoi(ev); if (v > 0) N_DIRECT = v; } }
    // Memory gate: a full symmetric decomposition needs the operator + eigenvectors +
    // LAPACK workspace + read copies -- budget ~5 dense m x m doubles. Take the direct
    ...
    { const char* ev = std::getenv("MGCCA_EIGEN_MAX_MB");
      if (ev && *ev) { double v = std::atof(ev); if (v > 0) budget_mb = v; } }
    const bool mem_ok = (est_mb <= budget_mb);
    int keig = std::min(std::max(nfac + 1, 2), (int)m);

    // Relative-residual with a denominator guard: when ||S V|| is numerically zero the
    // ratio is undefined -> return 0 only if the numerator is also numerically zero,
    // else NA (this is exactly the degenerate / near-null operator regime we diagnose).
    auto rel_resid = [](double num, double den) -> double {
        const double tol = 1e-30;
        if (den <= tol) return (num <= tol) ? 0.0 : R_NaReal;
        return num / den;
    };
    // Block-wise eigen-residual ||S V - V diag(vals)||_F / ||S V||_F over the top nfac,
    // reading S = MKsum05 row-block by row-block so S is never fully materialized. Used
    // on the iterative path (where partial convergence is the real risk) instead of NaN.
    auto resid_blockwise = [&](const Eigen::MatrixXd& Vn, const Eigen::VectorXd& vals) -> double {
        std::unique_ptr<BigDataStatMeth::hdf5Dataset> dS(
            new BigDataStatMeth::hdf5Dataset(filename, tmp_group, "MKsum05", false));
        dS->openDataset();
        const std::size_t bud = 4000000;
        ...
        for (std::size_t r = 0; r < m; r += br0) {
            ...
            std::vector<hsize_t> off = {0, (hsize_t)r}, cnt = {(hsize_t)m, (hsize_t)br},
                                 st = {1, 1}, bl = {1, 1};
            dS->readDatasetBlock(off, cnt, st, bl, Sb.begin());
            Eigen::Map<Eigen::MatrixXd> Sblk(Sb.begin(), (Eigen::Index)br, (Eigen::Index)m);
            Eigen::MatrixXd SV = Sblk * Vn;                          // br x nfac
            Eigen::MatrixXd resid = SV - Vn.middleRows((Eigen::Index)r, (Eigen::Index)br)
                                             * vals.asDiagonal();
            num2 += resid.squaredNorm(); den2 += SV.squaredNorm();
        }
        return rel_resid(std::sqrt(num2), std::sqrt(den2));
    };

    // Writes values (descending, 1 x keig) + vectors (m x keig) + the validity
    // diagnostics: external gap mu_L - mu_{L+1} (absolute + relative to mu_L) and, when
    // available, the eigen-residual ||S V - V diag||_F / ||S V||_F over the top nfac.
    // A small gap => the L-dim subspace is not statistically identifiable even though
    // the solver converged (computational convergence != identifiability).
    auto write_results = [&](const Eigen::VectorXd& valk, const Eigen::MatrixXd& Vk,
                             double reig) {
        write_full(filename, "EIGEN/MKsum05", "values", Eigen::MatrixXd(valk.transpose()), 0);
        write_full(filename, "EIGEN/MKsum05", "vectors", Vk, 0);
        const bool have = ((int)valk.size() > nfac);
        double gabs = have ? (valk(nfac - 1) - valk(nfac)) : R_NaReal;
        double grel = have ? gabs / std::max(std::abs(valk(nfac - 1)), 1e-300) : R_NaReal;
        Eigen::MatrixXd ga(1, 1), gr(1, 1), re(1, 1);
        ga(0, 0) = gabs; gr(0, 0) = grel; re(0, 0) = reig;
        write_full(filename, "EIGEN/MKsum05", "eiggap", ga, 0);
        write_full(filename, "EIGEN/MKsum05", "eiggap_rel", gr, 0);
        write_full(filename, "EIGEN/MKsum05", "eig_residual", re, 0);
    };
    auto solve_direct = [&]() {
        Eigen::MatrixXd Sop = read_full(filename, tmp_group, "MKsum05");   // m x m
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> es(Sop);
        if (es.info() != Eigen::Success)
            throw std::runtime_error("run_eigen: SelfAdjointEigenSolver did not succeed");
        const Eigen::VectorXd ea = es.eigenvalues();      // ascending
        const Eigen::MatrixXd ua = es.eigenvectors();
        const int mm = (int)ea.size();
        Eigen::MatrixXd Vk(mm, keig); Eigen::VectorXd valk(keig);
        for (int c = 0; c < keig; ++c) { valk(c) = ea(mm - 1 - c); Vk.col(c) = ua.col(mm - 1 - c); }
        Eigen::MatrixXd Vn = Vk.leftCols(nfac);
        Eigen::MatrixXd SV = Sop * Vn;
        double reig = rel_resid((SV - Vn * valk.head(nfac).asDiagonal()).norm(), SV.norm());
        write_results(valk, Vk, reig);
    };

    if ((int)m <= N_DIRECT && mem_ok) {
        solve_direct();                                   // default for the mgcca domain
    } else {
        bool ok = true;                                   // large operator: out-of-core
        try {
            BigDataStatMeth::RcppbdEigen_hdf5(filename, tmp_group, "MKsum05", keig, "LM", 0,
                                              false, false, 1e-10, 1000, true, true, threads);
        } catch (...) { ok = false; }
        if (!ok) {
            // Iterative failed (typically a near-degenerate operator). Retry directly
            // ONLY if it fits the RAM budget; otherwise error with the diagnostic rather
            // than risk an out-of-memory in an unconditional fallback.
            if (!mem_ok)
                throw std::runtime_error("run_eigen: iterative eigensolver did not converge "
                    "(near-degenerate operator) and the m x m direct fallback exceeds the RAM "
                    "budget; raise MGCCA_EIGEN_MAX_MB or use an appropriately scaled ridge");
            solve_direct();
        } else {
            Eigen::MatrixXd rv = read_full(filename, "EIGEN/MKsum05", "values");
            Eigen::Map<Eigen::VectorXd> vf(rv.data(), rv.size());
            Eigen::VectorXd valk = vf.head(std::min((int)vf.size(), keig));
            Eigen::MatrixXd Vk = read_full(filename, "EIGEN/MKsum05", "vectors");
            double reig = resid_blockwise(Vk.leftCols(nfac), valk.head(nfac));   // block-wise
            write_results(valk, Vk, reig);
        }
    }
...
inline void run_cor_ave(const std::string& filename, const std::string& tmp_group,
    ...
    for (std::size_t j = 0; j < J; ++j) {
        ...
        std::vector<int> pres = present_rows(filename, tmp_group, ds);
        const int npr = (int)pres.size();
        // corsY_j is a correlation over the individuals PRESENT in block j, so its
        // p-value carries n_present - 2 degrees of freedom. Using the union size m
        // (as this stage did before 1.1.0) inflates the df wherever a block has
        // missing individuals and makes the test anticonservative.
        const double df = (double)npr - 2.0;
        if (df <= 0.0)
            throw std::runtime_error("run_cor_ave: block '" + ds + "' has fewer than "
                                     "three present individuals; no p-value is defined");
...
inline void run_scores(const std::string& filename, const std::string& tmp_group,
    ...
    for (std::size_t j = 0; j < J; ++j) {
        ...
        std::vector<int> pres = present_rows(filename, tmp_group, ds);
        Eigen::MatrixXd As = scale_by_present_sd(A, KXA, pres, nfac, ds);
        ...
        mark_absent_scores(filename, final_group, ds, pres, rn, comp);
    }
}
